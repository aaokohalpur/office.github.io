...
function englishToNepaliNumber(number) {
    ...
    const devanagariToEnglish = {
      ...
      function convertToEnglish(devanagari) {
        return devanagari
          .split("")
          .map((char) => devanagariToEnglish[char] || char)
          .join("");
      }
      ...
      function convertToDevanagari(num) {
        return num
          .split("")
          .map((char) => englishToDevanagari[char] || char)
          .join("");
      }

      function autoConvertToDevanagari(event) {
        const inputField = event.target;
        const value = inputField.value;
        const convertedValue = convertToDevanagari(convertToEnglish(value));
      ...
      function calculateDOB() {
        // Get input values
        const age = parseInt(
          convertToEnglish(document.getElementById("age").value)
        );
        const currentDate = convertToEnglish(
          document.getElementById("currentDate").value
        );

        // Validate inputs
        if (isNaN(age) || !currentDate.match(/^\d{4}-\d{2}-\d{2}$/)) {
          document.getElementById("result").innerText =
            "कृपया उमेर र नागरिकता जारी मिति टाइप गर्नुहोस्।";
          return;
        }
        ...
        if (
          ...
          document.getElementById("result").innerText =
            "कृपया जारी मिति yyyy-mm-dd फर्मेतमा टाइप गर्नुहोस्।";
          return;
        }
        ...
        const dateOfBirth = `${birthYear}-${String(month).padStart(
          2,
          "0"
        ...
        document.getElementById(
          "result"
        ).innerText = `तपाईको जन्म मिति: ${convertToDevanagari(
          dateOfBirth
        )} रहेको छ ।`;
      }
      ...
      function calculateDOB1() {
        // Get input values
        const birthYear = parseInt(
          convertToEnglish(document.getElementById("birthYear").value)
        );
        const currentDate1 = convertToEnglish(
          document.getElementById("currentDate1").value
        );

        // Validate inputs
        if (isNaN(birthYear) || !currentDate1.match(/^\d{4}-\d{2}-\d{2}$/)) {
        ...
        // Parse the current date
        const [year, month, day] = currentDate1.split("-").map(Number);
        ...
        if (
          ...
          document.getElementById("result1").innerText =
            "कृपया yyyy-mm-dd प्रारूप में सही तिथि दर्ज करें।";
          return;
        }

        // Calculate the date of birth based on the current date and the provided birth year
        // Assuming birth year entered is the same year as current year
